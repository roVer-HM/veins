...
PersonSubscriptionManager::PersonSubscriptionManager(std::shared_ptr<TraCIConnection>& connection, std::shared_ptr<TraCICommandInterface>& commandInterface)
    : SubscriptionManagerBase(connection, commandInterface)
    , updatedPersons()
    , disappearedPersons()
{
    subscribeToPersonIDList();
}

void PersonSubscriptionManager::updateWithList(std::list<std::string>& currentlyActivePersonIds)
{
...
bool PersonSubscriptionManager::update(TraCIBuffer& buffer)
{
        ...
        if (variableStatus == TraCIConstants::RTYPE_OK) {
            ...
            if (isSubscribed(responseObjectID)) {
                if (variableStatus == TraCIConstants::RTYPE_NOTIMPLEMENTED) {
                    throw cRuntimeError("TraCI server reported subscribing to vehicle variable 0x%2x not implemented (\"%s\"). Might need newer version.", responseVariableID, errormsg.c_str());
                }

                throw cRuntimeError("TraCI server reported error subscribing to vehicle variable 0x%2x (\"%s\").", responseVariableID, errormsg.c_str());
            }
        }
    }
    ...
    if (isSubscribed(responseObjectID)) {
        ...
        person.speed = speed;
        person.angle = angle;
        person.id = responseObjectID;
        person.typeID = typeID;
        updatedPersons.push_back(person);
    }

    return idListUpdateReceived;
}

std::vector<Person> PersonSubscriptionManager::getUpdated() const
{
    return updatedPersons;
}

std::vector<std::string> PersonSubscriptionManager::getDisappeared() const
{
    std::vector<std::string> output;
    std::copy(disappearedPersons.begin(), disappearedPersons.end(), std::back_inserter(output));
    return output;
}

void PersonSubscriptionManager::subscribeToPersonIDList() {
...
void PersonSubscriptionManager::processPersonIDList(std::list<std::string>& idList)
{
    ...
    std::set_difference(idList.begin(), idList.end(), subscribed.begin(), subscribed.end(), std::back_inserter(needSubscribe));
    ...
    std::set_difference(subscribed.begin(), subscribed.end(), idList.begin(), idList.end(), std::inserter(disappearedPersons, disappearedPersons.begin()));
    for (auto id : disappearedPersons) {
        if (isSubscribed(id)) {
...
void PersonSubscriptionManager::subscribeToPersonVariables(std::string id)
{
    ...
    TraCIBuffer buffer = getConnection()->query(TraCIConstants::CMD_SUBSCRIBE_PERSON_VARIABLE, TraCIBuffer() << beginTime << endTime << objectId << variableNumber << variable1 << variable2 << variable3 << variable4 << variable5);
...
void PersonSubscriptionManager::clearAPI() {
    updatedPersons.clear();
    disappearedPersons.clear();
}
